...
fn main() -> anyhow::Result<(), Box<dyn Error>> {
    let args = Args::parse_with_defaults("new-sdk-sync-consumer");
    Registry::default()
        .with(tracing_subscriber::fmt::layer())
        .with(EnvFilter::try_from_default_env().unwrap_or(EnvFilter::new("INFO")))
        .init();
    info!(
        "New SDK sync consumer has started, selected transport: {}",
        args.transport
    );
    ...
    let name = "new-sdk-sync-consumer";
    ...
    consumer.init()?;
    consume_messages(&args, &mut consumer)?;

    Ok(())
}

pub fn consume_messages(args: &Args, consumer: &mut IggyConsumer) -> Result<(), Box<dyn Error>> {
    ...
    // Use Iterator::next() instead of StreamExt::next()
