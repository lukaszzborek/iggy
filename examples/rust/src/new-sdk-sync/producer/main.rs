...
fn main() -> anyhow::Result<(), Box<dyn Error>> {
    let args = Args::parse_with_defaults("new-sdk-sync-producer");
    Registry::default()
        .with(tracing_subscriber::fmt::layer())
        .with(EnvFilter::try_from_default_env().unwrap_or(EnvFilter::new("INFO")))
        .init();
    info!(
        "New SDK sync producer has started, selected transport: {}",
        args.transport
    );

    let client_provider_config = Arc::new(ClientProviderConfig::from_args(args.to_sdk_args())?);
    let client = client_provider::get_raw_connected_client(client_provider_config)?;
    ...
    producer.init()?;
    produce_messages(&args, &producer)?;
    Ok(())
}

fn produce_messages(args: &Args, producer: &IggyProducer) -> anyhow::Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        // Sleep between batches if interval is specified
        if let Some(interval) = interval {
            std::thread::sleep(interval.get_duration());
        }
    }
}
