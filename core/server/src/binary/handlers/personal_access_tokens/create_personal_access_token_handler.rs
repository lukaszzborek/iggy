...
impl ServerCommandHandler for CreatePersonalAccessToken {
    ...
    async fn handle(
        ...
        let (personal_access_token, token) = match shard
            ...
            .await?
        {
            ShardSendRequestResult::Recoil(message) => {
                        ...
                        payload
                {
                    shard
                        ...
                        .error(|e: &IggyError| {
                            format!(
                                "{COMPONENT} (error: {e}) - failed to create personal access token with name: {name}, session: {session}"
                            )
                        })?
                } else {
                    unreachable!(
            ...
            ShardSendRequestResult::Response(response) => match response {
                ShardResponse::CreatePersonalAccessTokenResponse(pat, token) => (pat, token),
