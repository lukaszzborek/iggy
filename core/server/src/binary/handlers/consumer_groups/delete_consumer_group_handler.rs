    ...
    async fn handle(
            ...
            .await?
        {
            ShardSendRequestResult::Recoil(message) => {
                ...
                {
                    shard.delete_consumer_group(session, &stream_id, &topic_id, &group_id).error(|e: &IggyError| {
                        format!(
            ...
            ShardSendRequestResult::Response(response) => match response {
                ShardResponse::DeleteConsumerGroupResponse => {
                    ...
                    shard
                        .state
                        .apply(session.get_user_id(), &EntryCommand::DeleteConsumerGroup(self))
                        .await
                        .error(|e: &IggyError| {
                            format!(
                                "{COMPONENT} (error: {e}) - failed to apply delete consumer group for stream_id: {stream_id}, topic_id: {topic_id}, session: {session}"
                            )
                        })?;
