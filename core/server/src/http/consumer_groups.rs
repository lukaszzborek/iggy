...
async fn get_consumer_group(
    ...
    let numeric_group_id = shard
        .metadata
        .get_consumer_group_id(numeric_stream_id, numeric_topic_id, &identifier_group_id)
        .ok_or(CustomError::ResourceNotFound)?;

    shard.permissioner.get_consumer_group(
        session.get_user_id(),
        numeric_stream_id,
        numeric_topic_id,
    )?;
...
async fn create_consumer_group(
    ...
    let group_id = state.shard.create_consumer_group(
        &session,
        &command.stream_id,
        &command.topic_id,
        command.name.clone(),
    ...
    let consumer_group_details = {
        ...
        mapper::map_consumer_group_details_from_metadata(cg_meta)
    };
