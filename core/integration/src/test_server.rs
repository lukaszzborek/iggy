    ...
    fn watchdog_loop(
        ...
        loop {
            ...
            if !process_alive {
                ...
                // We print to stderr and abort because panic in a spawned thread
    ...
    fn is_process_alive(pid: u32) -> bool {
        ...
        match fs::read_to_string(&stat_path) {
            Ok(content) => {
                ...
                // State is the third field after the command name in parentheses
                // States: R=running, S=sleeping, D=disk sleep, Z=zombie, T=stopped, etc.
            ...
            Err(_) => false, // Can't read /proc/{pid}/stat, process doesn't exist
        }
    }
