...
impl SystemClient for ClientWrapper {
    async fn get_stats(&self) -> Result<Stats, IggyError> {
        match self {
            ...
            ClientWrapper::TcpTokio(client) => client.get_stats().await,
        }
    }
    ...
    async fn get_me(&self) -> Result<ClientInfoDetails, IggyError> {
        match self {
            ...
            ClientWrapper::TcpTokio(client) => client.get_me().await,
        }
    }
    ...
    async fn heartbeat_interval(&self) -> IggyDuration {
        match self {
            ...
            ClientWrapper::TcpTokio(client) => client.heartbeat_interval().await,
        }
    }
    ...
    async fn snapshot(
        ...
        match self {
            ...
            ClientWrapper::TcpTokio(client) => client.snapshot(compression, snapshot_types).await,
        }
    }
}
