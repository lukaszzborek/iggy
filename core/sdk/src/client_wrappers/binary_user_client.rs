...
pub mod sync_impl {
    ...
    impl UserClient for ClientWrapper {
        fn get_user(&self, user_id: &Identifier) -> Result<Option<UserInfoDetails>, IggyError> {
            match self {
                ClientWrapper::Tcp(client) => client.get_user(user_id),
                ClientWrapper::TcpTls(client) => client.get_user(user_id),
                ClientWrapper::Iggy(_) => Err(IggyError::InvalidConfiguration),
            }
        }

        fn get_users(&self) -> Result<Vec<UserInfo>, IggyError> {
            match self {
                ClientWrapper::Tcp(client) => client.get_users(),
                ClientWrapper::TcpTls(client) => client.get_users(),
                ClientWrapper::Iggy(_) => Err(IggyError::InvalidConfiguration),
            }
        }
        ...
        fn create_user(
            ...
            match self {
                ClientWrapper::Tcp(client) => {
                    client.create_user(username, password, status, permissions)
                }
        ...
        fn delete_user(&self, user_id: &Identifier) -> Result<(), IggyError> {
            match self {
                ClientWrapper::Tcp(client) => client.delete_user(user_id),
                ClientWrapper::TcpTls(client) => client.delete_user(user_id),
                ClientWrapper::Iggy(_) => Err(IggyError::InvalidConfiguration),
            }
        }
        ...
        fn update_user(
            ...
            match self {
                ClientWrapper::Tcp(client) => client.update_user(user_id, username, status),
                ClientWrapper::TcpTls(client) => client.update_user(user_id, username, status),
                ClientWrapper::Iggy(_) => Err(IggyError::InvalidConfiguration),
            }
        }
        ...
        fn update_permissions(
            ...
            match self {
                ClientWrapper::Tcp(client) => client.update_permissions(user_id, permissions),
                ClientWrapper::TcpTls(client) => client.update_permissions(user_id, permissions),
                ClientWrapper::Iggy(_) => Err(IggyError::InvalidConfiguration),
            }
        }
        ...
        fn login_user(&self, username: &str, password: &str) -> Result<IdentityInfo, IggyError> {
            match self {
                ClientWrapper::Tcp(client) => client.login_user(username, password),
                ClientWrapper::TcpTls(client) => client.login_user(username, password),
                ClientWrapper::Iggy(_) => Err(IggyError::InvalidConfiguration),
            }
        }

        fn logout_user(&self) -> Result<(), IggyError> {
            match self {
                ClientWrapper::Tcp(client) => client.logout_user(),
                ClientWrapper::TcpTls(client) => client.logout_user(),
                ClientWrapper::Iggy(_) => Err(IggyError::InvalidConfiguration),
            }
        }
    }
