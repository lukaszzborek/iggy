...
use iggy_common::{ConnectionStringUtils, TcpClientConfigBuilder, TransportProtocol};
use std::sync::Arc;

use crate::prelude::IggyClient;
use crate::prelude::{ClientWrapper, IggyClientBuilder, TcpClient, TcpTlsClient};
use iggy_common::IggyError;

impl IggyClientBuilder {
    /// Creates a new `IggyClientBuilder` from the provided connection string.
    /// In sync mode, only TCP transport is supported.
    pub fn from_connection_string(connection_string: &str) -> Result<Self, IggyError> {
        let mut builder = Self::new();

        match ConnectionStringUtils::parse_protocol(connection_string)? {
            TransportProtocol::Tcp => {
                // Default to plain TCP (non-TLS) when using connection string
...
impl TcpClientBuilder {
    ...
    pub fn build(self) -> Result<IggyClient, IggyError> {
        let tcp_config = Arc::new(self.config.build()?);

        // Choose between TCP and TLS based on config
        let client_wrapper = if tcp_config.tls_enabled {
            ClientWrapper::TcpTls(TcpTlsClient::create(tcp_config)?)
        } else {
            ClientWrapper::Tcp(TcpClient::create(tcp_config)?)
        };

        let client = self.parent_builder.with_client(client_wrapper).build()?;
        Ok(client)
    }
}
