...
impl<'a, R: crate::runtime::RuntimeExecutor> IggyConsumerMessageExt<'a> for IggyConsumerInner<R> {
        ...
        S: Into<ShutdownRx> + Send,
    {
        ...
        loop {
            if shutdown_rx.try_now() {
                info!(
                    "Received shutdown signal, stopping message consumption from consumer {name} on topic: {topic} and stream: {stream}",
                    name = self.name(),
                    topic = self.topic(),
                    stream = self.stream()
                );
            ...
            match self.next() {
                Some(Ok(received_message)) => {
                    process_message(self, message_consumer, received_message, &strategy);
                }
                Some(Err(err)) => {
